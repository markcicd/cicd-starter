pipeline {
    agent any
    environment {
        TF_VAR_gcp_project = "qwiklabs-gcp-01-ea3db84121f0"
        TF_VAR_bucket = "tf-remote-state-student_04_65a21aa7a648-16993-14394"
        REPOSITORY = "https://github.com/markcicd/cicd-starter"
            ...
            steps {
                dir("ansible") {
                    withCredentials([sshUserPrivateKey(credentialsId: "SSH_KEY", keyFileVariable: "ANSIBLE_PRIVATE_KEY_FILE"), file(credentialsId: 'gcp-svc-acct', variable: 'GOOGLE_CLOUD_KEYFILE_JSON')]) {
                        sh '''
                        export GOOGLE_APPLICATION_CREDENTIALS=$GOOGLE_CLOUD_KEYFILE_JSON
                        ansible 127.0.0.1 -m template -a 'src=${WORKSPACE}/ansible/inventory_template.yml dest=${WORKSPACE}/ansible/inventory.gcp_compute.yml' -e 'project_id=${TF_VAR_gcp_project}'
